...
static void playlist_load_ins_file(char *filename, char *playlist_name, long pos, char *title,
                                   int len);
        ...
        *set_info_text = TRUE;
        ...
        if (g_list_index(get_playlist(), entry) == -1)
            return;
               ...
               g_list_position(playlist, playing_song) > g_list_position(playlist, node))
        *set_info_text = TRUE;
...
void playlist_check_pos_current(void)
{
    ...
    bottom = MAX(0, __get_playlist_length() - playlistwin_list->pl_num_visible);
...
char *playlist_get_info_text(void)
{
    ...
    if (playlist_position->length != -1)
        length = g_strdup_printf(" (%d:%-2.2d)", playlist_position->length / 60000,
...
static guint playlist_load_ins(char *filename, long pos)
{
    ...
    if (ext && !strcasecmp(ext, ".pls")) {
        ...
        for (i = 1; i <= noe; i++) {
            ...
            line = read_ini_string_no_comment(filename, "playlist", key);
            g_snprintf(key, 15, "Title%d", i);
            if (cfg.use_pl_metadata)
                ext_title = read_ini_string_no_comment(filename, "playlist", key);
            else
                ext_title = NULL;
            if (line != NULL) {
                playlist_load_ins_file(line, filename, pos, ext_title, -1);
                entries++;
                if (pos >= 0)
                    pos++;
                g_free(line);
...
static void *playlist_get_info_func(void *arg)
{
    ...
    while (playlist_get_info_going) {
        ...
        } else if (!cfg.get_info_on_load && cfg.get_info_on_demand && cfg.playlist_visible &&
                   !cfg.playlist_shaded) {
...
void playlist_start_get_info_thread(void)
{
    ...
    pthread_create(&playlist_get_info_thread, NULL, playlist_get_info_func, NULL);
}
...
void playlist_remove_dead_files(void)
{
    ...
    while (node) {
        ...
        if (entry && entry->filename && !strstr(entry->filename, "://") && /* Don't kill URL's */
            ((temp = input_scan_dir(entry->filename)) == NULL) && access(entry->filename, R_OK)) {
            gboolean set_info_text = FALSE, restart_playing = FALSE;
