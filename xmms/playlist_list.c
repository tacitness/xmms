...
{
    if (event->button == 1 && pl->pl_fheight && inside_widget(event->x, event->y, &pl->pl_widget)) {
        int nr, y;
...
void playlist_list_draw(Widget *w)
{
    ...
    for (i = pl->pl_first; list && i < pl->pl_first + pl->pl_num_visible; list = list->next, i++) {
        ...
        if (pos != -1)
            sprintf(qstr, "|%d|%s", pos + 1, entry->length != -1 ? " " : "");
