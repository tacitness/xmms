...
static void convert_to_s16_ne(AFormat fmt, gpointer ptr, gint16 *left, gint16 *right, int nch,
                              int max)
{
    gint16 *ptr16;
    guint16 *ptru16;
...
void input_add_vis_pcm(int time, AFormat fmt, int nch, int length, void *ptr)
{
    ...
    max = length / nch;
    if (fmt == FMT_U16_LE || fmt == FMT_U16_BE || fmt == FMT_U16_NE || fmt == FMT_S16_LE ||
        fmt == FMT_S16_BE || fmt == FMT_S16_NE)
        max /= 2;
    max = CLAMP(max, 0, 512);
...
gboolean input_check_file(gchar *filename)
{
    ...
    while (node) {
        ...
        if (ip && !g_list_find(disabled_iplugins, ip) && ip->is_our_file(filename))
            return TRUE;
        node = node->next;
    }
    return FALSE;
...
void input_play(char *filename)
{
    ...
    if (get_current_output_plugin() == NULL) {
        xmms_show_message(_("No output plugin"), _("No output plugin has been selected"), _("OK"),
                          FALSE, NULL, NULL);
        mainwin_stop_pushed();
        return;
    }
...
void input_get_volume(int *l, int *r)
{
    ...
    *r = -1;
    if (get_input_playing()) {
        if (get_current_input_plugin() && get_current_input_plugin()->get_volume) {
...
{
    if (get_input_playing()) {
        if (get_current_input_plugin() && get_current_input_plugin()->set_volume) {
