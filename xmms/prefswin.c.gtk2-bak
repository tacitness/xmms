...
void prefswin_ilist_clicked(GtkCList *clist, gint row, gint column, GdkEventButton *event,
                            gpointer data)
{
    InputPlugin *ip;
    gint index;
    ...
    if (clist->selection) {
        ...
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(prefswin_audio_ie_cbox),
                                     (g_list_find(disabled_iplugins, ip) ? FALSE : TRUE));
...
void prefswin_glist_clicked(GtkCList *clist, gint row, gint column, GdkEventButton *event,
                            gpointer data)
{
    GeneralPlugin *gp;
    gint index;
...
void prefswin_vlist_clicked(GtkCList *clist, gint row, gint column, GdkEventButton *event,
                            gpointer data)
{
    VisPlugin *vp;
    gint index;
...
void prefswin_elist_clicked(GtkCList *clist, gint row, gint column, GdkEventButton *event,
                            gpointer data)
{
    EffectPlugin *ep;
    gint index;
...
void prefswin_apply_changes(void)
{
    ...
    cfg.snap_distance =
        CLAMP(atoi(gtk_entry_get_text(GTK_ENTRY(prefswin_options_sd_entry))), 0, 1000);
    ...
    cfg.pause_between_songs_time =
        CLAMP(atoi(gtk_entry_get_text(GTK_ENTRY(prefswin_options_pbs_entry))), 0, 1000);
    cfg.mouse_change =
...
void prefswin_font_browse_cb(GtkWidget *w, gpointer data)
{
    ...
    gtk_font_selection_dialog_set_font_name(GTK_FONT_SELECTION_DIALOG(fontsel),
                                            gtk_entry_get_text(
                                                GTK_ENTRY(prefswin_options_font_entry)));
    gtk_signal_connect(GTK_OBJECT(GTK_FONT_SELECTION_DIALOG(fontsel)->ok_button), "clicked",
                       GTK_SIGNAL_FUNC(prefswin_font_browse_ok), fontsel);
    gtk_signal_connect_object(GTK_OBJECT(GTK_FONT_SELECTION_DIALOG(fontsel)->cancel_button),
                              "clicked", GTK_SIGNAL_FUNC(gtk_widget_destroy), GTK_OBJECT(fontsel));
    gtk_signal_connect(GTK_OBJECT(fontsel), "destroy", GTK_SIGNAL_FUNC(gtk_widget_destroyed),
                       &fontsel);
...
void prefswin_mainwin_font_browse_cb(GtkWidget *w, gpointer data)
{
    ...
    if (!fontsel) {
        ...
        gtk_font_selection_dialog_set_font_name(GTK_FONT_SELECTION_DIALOG(fontsel),
                                                gtk_entry_get_text(
                                                    GTK_ENTRY(prefswin_mainwin_font_entry)));
        gtk_signal_connect(GTK_OBJECT(GTK_FONT_SELECTION_DIALOG(fontsel)->ok_button), "clicked",
                           GTK_SIGNAL_FUNC(prefswin_mainwin_font_browse_ok), fontsel);
        gtk_signal_connect_object(GTK_OBJECT(GTK_FONT_SELECTION_DIALOG(fontsel)->cancel_button),
...
static GtkWidget *prefswin_option_new_with_label_to_table(gboolean *cfg, char *label,
                                                          GtkTable *table, int x, int y)
{
    GtkWidget *buttonw;
...
void create_prefs_window(void)
{
    ...
    gtk_window_set_transient_for(GTK_WINDOW(prefswin), GTK_WINDOW(mainwin));
    gtk_signal_connect(GTK_OBJECT(prefswin), "delete_event", GTK_SIGNAL_FUNC(prefswin_delete_event),
                       NULL);
    ...
    gtk_signal_connect(GTK_OBJECT(prefswin_audio_ilist), "select_row",
                       GTK_SIGNAL_FUNC(prefswin_ilist_clicked), NULL);
    gtk_signal_connect(GTK_OBJECT(prefswin_audio_ilist), "unselect_row",
                       GTK_SIGNAL_FUNC(prefswin_ilist_clicked), NULL);
    ...
    gtk_signal_connect(GTK_OBJECT(prefswin_audio_iconfig), "clicked",
                       GTK_SIGNAL_FUNC(prefswin_iconfigure), prefswin_audio_ilist);
    ...
    gtk_signal_connect(GTK_OBJECT(prefswin_audio_iabout), "clicked",
                       GTK_SIGNAL_FUNC(prefswin_iabout), prefswin_audio_ilist);
    ...
    gtk_signal_connect(GTK_OBJECT(prefswin_audio_ie_cbox), "toggled",
                       GTK_SIGNAL_FUNC(prefswin_ip_toggled), prefswin_audio_ilist);
    ...
    gtk_signal_connect(GTK_OBJECT(prefswin_audio_oconfig), "clicked",
                       GTK_SIGNAL_FUNC(prefswin_oconfigure), NULL);
    ...
    gtk_signal_connect(GTK_OBJECT(prefswin_audio_oabout), "clicked",
                       GTK_SIGNAL_FUNC(prefswin_oabout), NULL);
    gtk_box_pack_start(GTK_BOX(prefswin_audio_ohbox), prefswin_audio_oabout, TRUE, TRUE, 0);

    gtk_notebook_append_page(GTK_NOTEBOOK(prefswin_notebook), prefswin_audio_vbox,
    ...
    gtk_signal_connect(GTK_OBJECT(prefswin_eplugins_list), "select_row",
                       GTK_SIGNAL_FUNC(prefswin_elist_clicked), NULL);
    gtk_signal_connect(GTK_OBJECT(prefswin_eplugins_list), "unselect_row",
                       GTK_SIGNAL_FUNC(prefswin_elist_clicked), NULL);
    scrolled_win = gtk_scrolled_window_new(NULL, NULL);
    ...
    gtk_signal_connect(GTK_OBJECT(prefswin_eplugins_config), "clicked",
                       GTK_SIGNAL_FUNC(prefswin_econfigure), prefswin_eplugins_list);
    ...
    gtk_signal_connect(GTK_OBJECT(prefswin_eplugins_about), "clicked",
                       GTK_SIGNAL_FUNC(prefswin_eabout), prefswin_eplugins_list);
    ...
    gtk_signal_connect(GTK_OBJECT(prefswin_eplugins_use_cbox), "toggled",
                       GTK_SIGNAL_FUNC(prefswin_eplugins_use_cb), prefswin_eplugins_list);
    gtk_box_pack_start(GTK_BOX(prefswin_eplugins_hbox), prefswin_eplugins_use_cbox, FALSE, FALSE,
                       10);

    gtk_notebook_append_page(GTK_NOTEBOOK(prefswin_notebook), prefswin_eplugins_frame,
                             gtk_label_new(_("Effects Plugins")));
    ...
    gtk_signal_connect(GTK_OBJECT(prefswin_gplugins_list), "select_row",
                       GTK_SIGNAL_FUNC(prefswin_glist_clicked), NULL);
    gtk_signal_connect(GTK_OBJECT(prefswin_gplugins_list), "unselect_row",
                       GTK_SIGNAL_FUNC(prefswin_glist_clicked), NULL);
    scrolled_win = gtk_scrolled_window_new(NULL, NULL);
    ...
    gtk_signal_connect(GTK_OBJECT(prefswin_gplugins_config), "clicked",
                       GTK_SIGNAL_FUNC(prefswin_gconfigure), prefswin_gplugins_list);
    ...
    gtk_signal_connect(GTK_OBJECT(prefswin_gplugins_about), "clicked",
                       GTK_SIGNAL_FUNC(prefswin_gabout), prefswin_gplugins_list);
    ...
    gtk_signal_connect(GTK_OBJECT(prefswin_gplugins_use_cbox), "toggled",
                       GTK_SIGNAL_FUNC(prefswin_gplugins_use_cb), prefswin_gplugins_list);
    gtk_box_pack_start(GTK_BOX(prefswin_gplugins_hbox), prefswin_gplugins_use_cbox, FALSE, FALSE,
                       10);

    gtk_notebook_append_page(GTK_NOTEBOOK(prefswin_notebook), prefswin_gplugins_frame,
    ...
    gtk_signal_connect(GTK_OBJECT(prefswin_vplugins_list), "select_row",
                       GTK_SIGNAL_FUNC(prefswin_vlist_clicked), NULL);
    gtk_signal_connect(GTK_OBJECT(prefswin_vplugins_list), "unselect_row",
                       GTK_SIGNAL_FUNC(prefswin_vlist_clicked), NULL);
    ...
    gtk_signal_connect(GTK_OBJECT(prefswin_vplugins_config), "clicked",
                       GTK_SIGNAL_FUNC(prefswin_vconfigure), prefswin_vplugins_list);
    ...
    gtk_signal_connect(GTK_OBJECT(prefswin_vplugins_about), "clicked",
                       GTK_SIGNAL_FUNC(prefswin_vabout), prefswin_vplugins_list);
    ...
    gtk_signal_connect(GTK_OBJECT(prefswin_vplugins_use_cbox), "toggled",
                       GTK_SIGNAL_FUNC(prefswin_vplugins_use_cb), prefswin_vplugins_list);
    gtk_box_pack_start(GTK_BOX(prefswin_vplugins_hbox), prefswin_vplugins_use_cbox, FALSE, FALSE,
                       10);

    gtk_notebook_append_page(GTK_NOTEBOOK(prefswin_notebook), prefswin_vplugins_box,
    ...
    gtk_tooltips_set_tip(prefswin_tooltips, options_giop,
                         _("Read song title and length only when starting to play"), NULL);
    gtk_box_pack_start(GTK_BOX(options_gi_box), options_giop, FALSE, FALSE, 0);
    options_giod = prefswin_radio_new_with_label(&cfg.get_info_on_demand,
                                                 GTK_RADIO_BUTTON(options_giop), _("demand"));
    gtk_tooltips_set_tip(prefswin_tooltips, options_giod,
    ...
    options_giol = prefswin_radio_new_with_label(&cfg.get_info_on_load,
                                                 GTK_RADIO_BUTTON(options_giop), _("load"));
    gtk_tooltips_set_tip(prefswin_tooltips, options_giol,
                         _("Read song title and length as soon as the song is loaded to the "
                           "playlist"),
                         NULL);
    ...
    prefswin_option_new_with_label_to_table(&cfg.allow_multiple_instances,
                                            _("Allow multiple instances"), GTK_TABLE(options_table),
                                            1, 0);

    prefswin_option_new_with_label_to_table(&cfg.convert_twenty, _("Convert %20 to space"),
                                            GTK_TABLE(options_table), 0, 1);
    opt = prefswin_option_new_with_label_to_table(&cfg.always_show_cb, _("Always show clutterbar"),
                                                  GTK_TABLE(options_table), 1, 1);
    gtk_tooltips_set_tip(prefswin_tooltips, opt,
                         _("The \"clutterbar\" is the row of buttons at the left side of the main "
                           "window"),
                         NULL);

    prefswin_option_new_with_label_to_table(&cfg.convert_underscore,
                                            _("Convert underscore to space"),
                                            GTK_TABLE(options_table), 0, 2);
    prefswin_option_new_with_label_to_table(&cfg.save_window_position, _("Save window positions"),
                                            GTK_TABLE(options_table), 1, 2);

    prefswin_option_new_with_label_to_table(&cfg.dim_titlebar, _("Dim titlebar when inactive"),
                                            GTK_TABLE(options_table), 0, 3);
    prefswin_option_new_with_label_to_table(&cfg.show_numbers_in_pl, _("Show numbers in playlist"),
                                            GTK_TABLE(options_table), 1, 3);

    prefswin_option_new_with_label_to_table(&cfg.sort_jump_to_file,
                                            _("Sort \"Jump to file\" alphabetically"),
                                            GTK_TABLE(options_table), 0, 4);
    prefswin_option_new_with_label_to_table(&cfg.eq_doublesize_linked,
                                            _("Equalizer doublesize linked"),
                                            GTK_TABLE(options_table), 1, 4);

    options_rt = prefswin_option_new_with_label_to_table(&cfg.use_realtime,
                                                         _("Use realtime priority when available"),
                                                         GTK_TABLE(options_table), 0, 5);
    gtk_tooltips_set_tip(prefswin_tooltips, options_rt,
                         _("Run XMMS with higher priority (not recommended)"), NULL);
    gtk_signal_connect(GTK_OBJECT(options_rt), "toggled", GTK_SIGNAL_FUNC(prefswin_rt_callback),
                       NULL);
    prefswin_option_new_with_label_to_table(&cfg.smooth_title_scroll, _("Smooth title scroll"),
                                            GTK_TABLE(options_table), 1, 5);

    options_pbs =
        prefswin_option_new_to_table(&cfg.pause_between_songs, GTK_TABLE(options_table), 0, 6);
    ...
    opt = prefswin_option_new_with_label_to_table(&cfg.use_backslash_as_dir_delimiter,
                                                  _("Use \'\\\' as a directory delimiter"),
                                                  GTK_TABLE(options_table), 1, 7);
    gtk_tooltips_set_tip(prefswin_tooltips, opt,
                         _("Recommended if you want to load playlists "
                           "that were created in MS Windows"),
                         NULL);
    ...
    gtk_notebook_append_page(GTK_NOTEBOOK(prefswin_notebook), prefswin_options_vbox,
    ...
    gtk_box_pack_start(GTK_BOX(prefswin_fonts_vbox), prefswin_fonts_playlist_frame, FALSE, FALSE,
                       0);
    ...
    gtk_signal_connect(GTK_OBJECT(prefswin_options_font_browse), "clicked",
                       GTK_SIGNAL_FUNC(prefswin_font_browse_cb), NULL);
    ...
    prefswin_mainwin_xfont =
        prefswin_option_new_with_label(&cfg.mainwin_use_xfont, _("Use X font"));
    ...
    gtk_box_pack_start(GTK_BOX(prefswin_mainwin_font_hbox), prefswin_mainwin_font_entry, TRUE, TRUE,
                       0);
    prefswin_mainwin_font_browse = gtk_button_new_with_label(_("Browse"));
    gtk_signal_connect(GTK_OBJECT(prefswin_mainwin_font_browse), "clicked",
                       GTK_SIGNAL_FUNC(prefswin_mainwin_font_browse_cb), NULL);
    gtk_widget_set_usize(prefswin_mainwin_font_browse, 85, 17);
    gtk_box_pack_start(GTK_BOX(prefswin_mainwin_font_hbox), prefswin_mainwin_font_browse, FALSE,
                       TRUE, 0);

    gtk_notebook_append_page(GTK_NOTEBOOK(prefswin_notebook), prefswin_fonts_vbox,
    ...
    gtk_notebook_append_page(GTK_NOTEBOOK(prefswin_notebook), prefswin_title_vbox,
    ...
    gtk_signal_connect(GTK_OBJECT(prefswin_cancel), "clicked", GTK_SIGNAL_FUNC(prefswin_cancel_cb),
                       NULL);
    ...
    gtk_signal_connect(GTK_OBJECT(prefswin_apply), "clicked", GTK_SIGNAL_FUNC(prefswin_apply_cb),
                       NULL);
...
void add_output_plugins(GtkOptionMenu *omenu)
{
    ...
    while (olist) {
        ...
        gtk_signal_connect(GTK_OBJECT(item), "activate", GTK_SIGNAL_FUNC(prefswin_output_cb),
                           GINT_TO_POINTER(i));
