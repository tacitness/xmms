...
static void read_1b_rgb(guint8 *input, gint input_size, guint8 *output, guint32 w, guint32 h,
                        struct rgb_quad *palette);
static void read_4b_rgb(guint8 *input, gint input_size, guint8 *output, guint32 w, guint32 h,
                        struct rgb_quad *palette);
static void read_4b_rle(guint8 *input, guint32 compr_size, guint8 *output, guint32 w, guint32 h,
                        struct rgb_quad *palette);
static void read_8b_rgb(guint8 *input, gint input_size, guint8 *output, guint32 w, guint32 h,
                        struct rgb_quad *palette);
static void read_8b_rle(guint8 *input, guint32 compr_size, guint8 *output, guint32 w, guint32 h,
                        struct rgb_quad *palette);
static void read_16b_rgb(guint8 *input, int input_size, guint8 *output, guint32 w, guint32 h,
                         struct bitfieldmask *mask);
static void read_24b_rgb(guint8 *input, gint input_size, guint8 *output, guint32 w, guint32 h);
static void read_32b_rgb(guint8 *input, int input_size, guint8 *output, guint32 w, guint32 h,
                         struct bitfieldmask *mask);
...
GdkPixmap *read_bmp(gchar *filename)
{
    ...
    if (!read_le_long(file, &offset) || !read_le_long(file, &headSize))
        goto failure;
    if (headSize == 12) /* BITMAPCOREINFO */
    {
        guint16 tmpw, tmph, dummy;

        if (!read_le_short(file, &tmpw) || !read_le_short(file, &tmph) ||
            !read_le_short(file, &dummy) || !read_le_short(file, &bitcount))
            goto failure;
        w = tmpw;
        h = tmph;
        imgsize = size - offset;
    ...
    } else if (headSize == 40) /* BITMAPINFO */
    {
        ...
        if (!read_le_long(file, &w) || !read_le_long(file, &h) || !read_le_short(file, &tmp) ||
            !read_le_short(file, &bitcount) || !read_le_long(file, &comp) ||
        ...
        g_warning("read_bmp(): Error in BMP file: Invalid header size: %d (%s)", headSize,
                  filename);
        goto failure;
    }
    if ((bitcount == 16 || bitcount == 32) && comp == BI_BITFIELDS) {
        if (offset - headSize - 14 >= 12) {
            if (!read_le_long(file, &mask.r) || !read_le_long(file, &mask.g) ||
                         ...
                         struct bitfieldmask *mask)
{
    ...
    for (y = 0; y < h; y++) {
        for (x = 0; x < w && input < (input_end - 3); x++) {
            guint32 rgb = *(input + 3) << 24 | *(input + 2) << 16 | *(input + 1) << 8 | *input;
            input += 4;
