...
gboolean xmms_fullscreen_init(GtkWidget *win)
{
    ...
    if (!fwin->display->modes) {
        XF86VidModeGetAllModeLines(fwin->display->display, DefaultScreen(fwin->display->display),
                                   &fwin->display->num_modes, &fwin->display->modes);

        if (!fwin->display->origmode) {
            XF86VidModeGetModeLine(fwin->display->display, DefaultScreen(fwin->display->display),
                                   &dummy, &origmode);
    ...
    if (!fwin->display->is_full && !fwin->is_full && fwin->display->can_full) {
        ...
        if (close < fwin->display->num_modes) {
            ...
            *h = fwin->display->modes[close]->vdisplay;
            ...
            XF86VidModeSetViewPort(fwin->display->display, DefaultScreen(fwin->display->display), 0,
                                   0);
