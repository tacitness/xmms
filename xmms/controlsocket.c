...
gboolean setup_ctrlsocket(void)
{
    ...
    if ((ctrl_fd = socket(AF_UNIX, SOCK_STREAM, 0)) != -1) {
        for (i = 0;; i++) {
            ...
            g_snprintf(saddr.sun_path, 108, "%s/xmms_%s.%d", g_get_tmp_dir(), g_get_user_name(), i);
            if (!xmms_remote_is_running(i)) {
                if ((unlink(saddr.sun_path) == -1) && errno != ENOENT) {
                    g_log(NULL, G_LOG_LEVEL_CRITICAL,
                          "setup_ctrlsocket(): Failed to unlink %s (Error: %s)", saddr.sun_path,
                          strerror(errno));
                }
            } else {
        ...
        g_log(NULL, G_LOG_LEVEL_CRITICAL, "setup_ctrlsocket(): Failed to open socket: %s",
              strerror(errno));
...
void check_ctrlsocket(void)
{
    ...
    for (pkt_list = packet_list; pkt_list; pkt_list = next) {
        ...
        case CMD_JUMP_TO_TIME:
            num = *((guint32 *)data);
            if (playlist_get_current_length() > 0 && num < playlist_get_current_length())
                input_seek(num / 1000);
            break;
        case CMD_SET_VOLUME:
            v[0] = ((guint32 *)data)[0];
