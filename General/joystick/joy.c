...
GeneralPlugin joy_gp = {NULL, /* handle */
                        NULL, /* filename */
                        -1,   /* xmms_session */
                        NULL, /* Description */
                        init, joy_about, joy_configure, cleanup};

joy_config joy_cfg = {10000,   NULL,    NULL,   JC_VOLUP, JC_VOLDWN, JC_PREV, JC_NEXT,
                      JC_FWD5, JC_BWD5, JC_RWD, JC_FWD,   0,         NULL};

/* ---------------------------------------------------------------------- */
GeneralPlugin *get_gplugin_info(void)
{
    joy_gp.description = g_strdup_printf(_("Joystick Control %s"), VERSION);
    return &joy_gp;
}
        ...
        if (select(max_fd, &set, NULL, NULL, &tv)) {
            if (FD_ISSET(joy_fd1, &set)) {
                ...
                case JS_EVENT_BUTTON:
                    if (js.number <= joy_cfg.num_buttons) {
                        js_command = joy_cfg.button_cmd[js.number];
                    }
                    break;
                case JS_EVENT_AXIS:
                    if (js.number == 0) {
