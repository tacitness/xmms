...
static gchar *ir_control[13] = {N_("Play"),  N_("Stop"),    N_("Pause"),    N_("Prev"),
                                N_("Next"),  N_("Vol +"),   N_("Seek -5s"), N_("Seek +5s"),
                                N_("Vol -"), N_("Shuffle"), N_("Repeat"),   N_("Playlist"),
                                N_("+100")};
static gchar *ir_playlist[10] = {N_("0"), N_("1"), N_("2"), N_("3"), N_("4"),
...
{
    gtk_entry_set_text(GTK_ENTRY(playlist_entry),
                       ircfg.playlist[gtk_spin_button_get_value_as_int(spin)]);
}

static void pl_entry_change_cb(GtkWidget *widget, GtkSpinButton *spin)
{
    ircfg.playlist[gtk_spin_button_get_value_as_int(spin)] =
        g_strdup(gtk_entry_get_text(GTK_ENTRY(playlist_entry)));
}

void ir_configure(void)
{
    GtkWidget *vbox, *notebook, *box, *frame, *table, *vbox2, *label, *button, *label_codelen;
    GtkAdjustment *adj;
    gint i;
