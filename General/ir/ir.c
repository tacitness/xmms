...
GeneralPlugin ir_gp = {NULL, /* handle */
                       NULL, /* filename */
                       -1,   /* xmms_session */
                       NULL, /* Description */
                       init, ir_about, ir_configure, cleanup};
...
/* A modified port initialization - seems to work every time */
void irapp_init_port(gchar *ir_port)
{
    gint i;
    ...
    while (keepGoing) {
        if (!irconf_is_going) {
            ...
            if (code) {
                ...
                if (!strcmp(text, ircfg.button_play) && g_timer_elapsed(timer1, NULL) > L_PAUSE) {
                ...
                else if (!strcmp(text, ircfg.button_stop) &&
                         g_timer_elapsed(timer1, NULL) > L_PAUSE) {
                    xmms_remote_stop(ir_gp.xmms_session);
                    g_timer_reset(timer1);
                    g_timer_reset(timer2);
                    g_timer_stop(timer2);
                    ir_hundreds = 0;
                } else if (!strcmp(text, ircfg.button_pause) &&
                           g_timer_elapsed(timer1, NULL) > L_PAUSE) {
                    ...
                    g_timer_stop(timer2);
                    ir_hundreds = 0;
                } else if (!strcmp(text, ircfg.button_shuffle) &&
                           g_timer_elapsed(timer1, NULL) > L_PAUSE) {
                    ...
                    g_timer_stop(timer2);
                    ir_hundreds = 0;
                } else if (!strcmp(text, ircfg.button_repeat) &&
                           g_timer_elapsed(timer1, NULL) > L_PAUSE) {
                    ...
                    g_timer_stop(timer2);
                    ir_hundreds = 0;
                } else if (!strcmp(text, ircfg.button_playlist) &&
                           g_timer_elapsed(timer1, NULL) > L_PAUSE) {
                    ...
                    g_timer_stop(timer2);
                    ir_hundreds = 0;
                } else if (!strcmp(text, ircfg.button_next) &&
                           g_timer_elapsed(timer1, NULL) > L_PAUSE) {
                    ...
                    g_timer_stop(timer2);
                    ir_hundreds = 0;
                } else if (!strcmp(text, ircfg.button_prev) &&
                           g_timer_elapsed(timer1, NULL) > L_PAUSE) {
                    ...
                    g_timer_stop(timer2);
                    ir_hundreds = 0;
                } else if (!strcmp(text, ircfg.button_seekf) &&
                           g_timer_elapsed(timer1, NULL) > S_PAUSE / 2) {
                    ...
                    g_timer_stop(timer2);
                    ir_hundreds = 0;
                } else if (!strcmp(text, ircfg.button_seekb) &&
                           g_timer_elapsed(timer1, NULL) > S_PAUSE / 2) {
                    output_time = xmms_remote_get_output_time(ir_gp.xmms_session);
                    if (output_time < 5000)
                        output_time = 5000;
                    ...
                    g_timer_stop(timer2);
                    ir_hundreds = 0;
                } else if (!strcmp(text, ircfg.button_volup) &&
                           g_timer_elapsed(timer1, NULL) > S_PAUSE) {
                    ...
                    g_timer_reset(timer1);
                } else if (!strcmp(text, ircfg.button_voldown) &&
                           g_timer_elapsed(timer1, NULL) > S_PAUSE) {
                    xmms_remote_get_volume(ir_gp.xmms_session, &vl, &vr);
                    if (vl < 5)
                        vl = 5;
                    ...
                    xmms_remote_set_volume(ir_gp.xmms_session, vl - 5, vr - 5);
                    g_timer_reset(timer1);
                } else if (!strcmp(text, ircfg.button_plus100) &&
                           g_timer_elapsed(timer1, NULL) > S_PAUSE) {
                    ir_hundreds += 1;
                    g_timer_reset(timer1);
                    g_timer_reset(timer2);
                            ...
                            if (ir_numpress >= 0) {
                                ...
                                if (ir_playlist_mode) {
                                    if (strcmp(ircfg.playlist[(10 * ir_numpress) + i], "")) {
                                        ir_playlist = NULL;
                                        ir_playlist =
                                            g_list_append(ir_playlist,
                                                          ircfg.playlist[(10 * ir_numpress) + i]);
