...
static void configure_ok_cb(GtkWidget *w, gpointer data)
{
    ...
    if (check_command(cmd) < 0 || check_command(cmd_after) < 0 || check_command(cmd_end) < 0)
        warn_user();
    else
...
static void configure(void)
{
    ...
    cmd_desc = gtk_label_new(_("Shell-command to run when xmms starts a new song."));
    ...
    cmd_after_desc = gtk_label_new(_("Shell-command to run toward the end of a song."));
    ...
    cmd_end_desc = gtk_label_new(_("Shell-command to run when xmms reaches the end "
                                   "of the playlist."));
    ...
    temp = g_strdup_printf(_("You can use the following format strings which "
...
static int timeout_func(gpointer data)
{
    ...
    if ((pos != previous_song || (!previous_file && current_file) ||
         (previous_file && !current_file) ||
         (previous_file && current_file && strcmp(previous_file, current_file))) &&
    ...
    if (!cmd_after_already_run && ((xmms_remote_get_playlist_time(sc_gp.xmms_session, pos) -
