...
static GSList *id3_get_content_v23(struct id3_frame *frame)
{
    ...
    input = id3_string_decode(ID3_TEXT_FRAME_ENCODING(frame), ID3_TEXT_FRAME_PTR(frame));
...
static GSList *id3_get_content_v24(struct id3_frame *frame)
{
    ...
    while (offset < frame->fr_size - 1) {
        ...
        input =
            id3_string_decode(ID3_TEXT_FRAME_ENCODING(frame), ID3_TEXT_FRAME_PTR(frame) + offset);
        ...
        if (endptr && endptr != input && *endptr == '\0' && num >= 0 && num < 256)
            list = push_element(list, num, NULL);
        ...
        offset +=
            id3_string_size(ID3_TEXT_FRAME_ENCODING(frame), ID3_TEXT_FRAME_PTR(frame) + offset);
    }
