...
static void label_set_text(GtkWidget *label, char *str, ...) G_GNUC_PRINTF(2, 3);
...
static void save_cb(GtkWidget *w, gpointer data)
{
    ...
    if ((fd = open(current_filename, O_RDWR)) != -1) {
        ...
        if (write(fd, &tag, sizeof(tag)) != sizeof(tag))
            msg = g_strdup_printf(_("%s\nUnable to write to file: %s"), _("Couldn't write tag!"),
                                  strerror(errno));
        close(fd);
    } else
        msg = g_strdup_printf(_("%s\nUnable to open file: %s"), _("Couldn't write tag!"),
                              strerror(errno));
    if (msg) {
        GtkWidget *mwin = xmms_show_message(_("File Info"), msg, _("OK"), FALSE, NULL, NULL);
...
static void remove_id3_cb(GtkWidget *w, gpointer data)
{
    ...
    if ((fd = open(current_filename, O_RDWR)) != -1) {
        ...
        msg = g_strdup_printf(_("%s\nUnable to open file: %s"), _("Couldn't remove tag!"),
                              strerror(errno));
    if (msg) {
        GtkWidget *mwin = xmms_show_message(_("File Info"), msg, _("OK"), FALSE, NULL, NULL);
        gtk_window_set_transient_for(GTK_WINDOW(mwin), GTK_WINDOW(window));
...
{
    static const char *channel_mode[] = {N_("Stereo"), N_("Joint stereo"), N_("Dual channel"),
                                         N_("Single channel")};
    if (mode < 0 || mode > 3)
        return "";
...
static void file_info_http(char *filename)
{
    ...
    if (mpg123_filename && !strcmp(filename, mpg123_filename) && mpg123_bitrate != 0) {
        set_mpeg_level_label(mpg123_mpeg25, mpg123_lsf, mpg123_layer);
        label_set_text(mpeg_bitrate, _("Bitrate: %d kb/s"), mpg123_bitrate);
        label_set_text(mpeg_samplerate, _("Samplerate: %d Hz"), mpg123_frequency);
    ...
    if ((fh = fopen(current_filename, "rb")) != NULL) {
        ...
        if (fread(&tag, 1, sizeof(tag), fh) == sizeof(tag)) {
            if (!strncmp(tag.tag, "TAG", 3)) {
                ...
                if (tag.u.v1_1.__zero == 0 && tag.u.v1_1.track_number > 0) {
                    ...
                    set_entry_tag(GTK_ENTRY(comment_entry), tag.u.v1_1.comment, 28);
                    gtk_entry_set_text(GTK_ENTRY(tracknum_entry), temp);
                    g_free(temp);
                } else {
                    set_entry_tag(GTK_ENTRY(comment_entry), tag.u.v1_0.comment, 30);
                    gtk_entry_set_text(GTK_ENTRY(tracknum_entry), "");
                }
        ...
        if (mpg123_get_xing_header(&xing_header, buf)) {
            ...
            label_set_text(mpeg_bitrate,
                           _("Bitrate: Variable,\n"
                             "avg. bitrate: %d kb/s"),
                           (int)((xing_header.bytes * 8) / (tpf * xing_header.frames * 1000)));
        } else {
        ...
        label_set_text(mpeg_flags,
                       _("%s\nError protection: %s\n"
                         "Copyright: %s\nOriginal: %s\n"
                         "Emphasis: %s"),
                       channel_mode_name(frm.mode), bool_label[frm.error_protection],
                       bool_label[frm.copyright], bool_label[frm.original], emphasis[frm.emphasis]);
