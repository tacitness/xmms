...
const int mpg123_freqs[9] = {44100, 48000, 32000, 22050, 24000, 16000, 11025, 12000, 8000};
...
static gboolean read_id3v2_tag(unsigned long head)
{
    ...
    if ((id3d = id3_open_mem(id3buf, 0)) != NULL) {
        ...
        if (!mpg123_info->first_frame) {
            ...
            mpg123_ip.set_info(mpg123_title, mpg123_length, mpg123_bitrate * 1000, mpg123_frequency,
                               mpg123_stereo);
            if (songname)
                g_free(songname);
        } else {
    ...
    if (!mpg123_head_check(newhead) || !mpg123_decode_header(fr, newhead)) {
        ...
        do {
            ...
            if (try > MAX_SKIP_LENGTH)
                return FALSE;
        } while (!mpg123_head_check(newhead) || !mpg123_decode_header(fr, newhead));
...
void mpg123_open_stream(char *bs_filenam, int fd)
{
    ...
    if (!strncasecmp(bs_filenam, "http://", 7)) {
        ...
        if ((filept = fopen(bs_filenam, "rb")) == NULL || stream_init() == -1)
