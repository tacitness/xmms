 ...
 */
static int III_get_side_info(struct III_sideinfo *si, int stereo, int ms_stereo, long sfreq,
                             int single, int lsf)
{
    int ch, gr;
...
static int III_dequantize_sample(real xr[SBLIMIT][SSLIMIT], int *scf, struct gr_info_s *gr_info,
 ...
 */
static void III_i_stereo(real xr_buf[2][SBLIMIT][SSLIMIT], int *scalefac, struct gr_info_s *gr_info,
...
#if 1
    int tab;
    static const real *tabs[3][2][2] = {{{tan1_1, tan2_1}, {tan1_2, tan2_2}},
 ...
 * III_hybrid
 */
static void III_hybrid(real fsIn[SBLIMIT][SSLIMIT], real tsOut[SSLIMIT][SBLIMIT], int ch,
                       struct gr_info_s *gr_info, struct frame *fr)
{
    static real block[2][2][SBLIMIT * SSLIMIT] = {{{
        0,
    }}};
    ...
    for (gr = 0; gr < granules; gr++) {
        ...
        if (mpg123_info->output_audio) {
            mpg123_ip.add_vis_pcm(mpg123_ip.output->written_time(),
                                  ...
                                  mpg123_cfg.channels == 2 ? fr->stereo : 1, mpg123_pcm_point,
                                  mpg123_pcm_sample);
            while (mpg123_ip.output->buffer_free() < mpg123_pcm_point && mpg123_info->going &&
