...
const char *mpg123_id3_genres[GENRE_MAX] = {N_("Blues"),
                                            N_("Classic Rock"),
                                            N_("Country"),
                                            N_("Dance"),
                                            N_("Disco"),
...
static void set_synth_functions(struct frame *fr)
{
    ...
    static func funcs[][3] = {{mpg123_synth_1to1, mpg123_synth_2to1, mpg123_synth_4to1},
                              {mpg123_synth_1to1_8bit, mpg123_synth_2to1_8bit,
                               mpg123_synth_4to1_8bit},
#ifdef USE_SIMD
                              {mpg123_synth_1to1_mmx, mpg123_synth_2to1, mpg123_synth_4to1},
                              {mpg123_synth_1to1_3dnow, mpg123_synth_2to1, mpg123_synth_4to1}
#endif
    };

    static func_mono funcs_mono[2][4] = {{mpg123_synth_1to1_mono, mpg123_synth_2to1_mono,
                                          mpg123_synth_4to1_mono},
                                         {mpg123_synth_1to1_8bit_mono, mpg123_synth_2to1_8bit_mono,
    ...
    if (CPU_HAS_3DNOW() && !p8 &&
        (mpg123_cfg.default_synth == SYNTH_3DNOW || mpg123_cfg.default_synth == SYNTH_AUTO)) {
        ...
        fr->synth_type = SYNTH_3DNOW;
               ...
               (mpg123_cfg.default_synth == SYNTH_MMX || mpg123_cfg.default_synth == SYNTH_AUTO)) {
...
static int is_our_file(char *filename)
{
    ...
    if (!strncasecmp(filename, "http://", 7)) { /* We assume all http:// (except those ending in
                                                   .ogg) are mpeg -- why do we do that? */
        ext = strrchr(filename, '.');
        if (ext) {
            if (!strncasecmp(ext, ".ogg", 4))
                return FALSE;
            if (!strncasecmp(ext, ".rm", 3) || !strncasecmp(ext, ".ra", 3) ||
                !strncasecmp(ext, ".rpm", 4) || !strncasecmp(ext, ".fla", 4) ||
                !strncasecmp(ext, ".flac", 5) || !strncasecmp(ext, ".ram", 4))
                return FALSE;
        }
        return TRUE;
    }
...
char *mpg123_format_song_title(struct id3v2tag_t *tag, char *filename)
{
    ...
    ret = xmms_get_titlestring(mpg123_cfg.title_override ? mpg123_cfg.id3_format
...
gboolean mpg123_get_first_frame(FILE *fh, struct frame *frm, guint8 **buffer)
{
    ...
    for (;;) {
        ...
        while (!mpg123_head_check(h) || !mpg123_decode_header(frm, h)) {
            if (!skip_id3v2(fh, h))
                return FALSE;
            if (!head_shift(fh, &h))
                return FALSE;
        ...
        if (mpg123_head_check(h2) && mpg123_decode_header(&tmp, h2) && (h & mask) == (h2 & mask)) {
...
static int mpg123_seek(struct frame *fr, xing_header_t *xh, gboolean vbr, int time)
{
    ...
    if (xh) {
        int percent = ((double)time * 100.0) / (mpg123_info->num_frames * mpg123_info->tpf);
        int byte = mpg123_seek_point(xh, percent);
        jumped = mpg123_stream_jump_to_byte(fr, byte);
    } else if (vbr && mpg123_length > 0) {
        int byte = ((guint64)time * 1000 * mpg123_info->filesize) / mpg123_length;
        jumped = mpg123_stream_jump_to_byte(fr, byte);
    } else {
...
static void *decode_loop(void *arg)
{
    ...
    if (!mpg123_info->eof && mpg123_info->going) {
        ...
        mpg123_ip.set_info(mpg123_title, mpg123_length, mpg123_bitrate * 1000,
        ...
        if (!mpg123_info->eof) {
            if (mpg123_read_frame(&fr) != 0) {
                ...
                if (mpg123_freqs[fr.sampling_frequency] != mpg123_frequency ||
                    ...
                    memcpy(&temp_fr, &fr, sizeof(struct frame));
                    if (mpg123_read_frame(&temp_fr) != 0) {
                        if (fr.sampling_frequency == temp_fr.sampling_frequency &&
                            temp_fr.stereo == fr.stereo) {
                            mpg123_ip.output->buffer_free();
                            mpg123_ip.output->buffer_free();
                            while (mpg123_ip.output->buffer_playing() && mpg123_info->going &&
                                   mpg123_info->jump_to_time == -1)
                                xmms_usleep(20000);
                            ...
                            mpg123_stereo = fr.stereo;
                            if (!mpg123_ip.output
                                     ->open_audio(mpg123_cfg.resolution == 16 ? FMT_S16_NE : FMT_U8,
...
static void aboutbox(void)
{
    ...
    aboutbox = xmms_show_message(_("About MPEG Layer 1/2/3 plugin"),
...
{
    mpg123_ip.description = g_strdup_printf(_("MPEG Layer 1/2/3 Player %s"), VERSION);
    return &mpg123_ip;
}
