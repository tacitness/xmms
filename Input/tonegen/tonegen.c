...
static void tone_about(void)
{
    ...
    box = xmms_show_message(_("About Tone Generator"),
...
static void *play_loop(void *arg)
{
    ...
    while (going) {
        for (i = 0; i < BUF_SAMPLES; i++) {
            ...
            data[i] = rint(((1 << 15) - 1) * (sum_sines / frequencies->len));
        }
        tone_ip.add_vis_pcm(tone_ip.output->written_time(), FMT_S16_NE, 1, BUF_BYTES, data);
...
static char *tone_title(char *filename)
{
    ...
    title = g_strdup_printf("%s %.1f Hz", _("Tone Generator: "), g_array_index(freqs, double, 0));
...
static InputPlugin tone_ip = {NULL, NULL,       NULL, /* Description */
                              NULL, tone_about, NULL,           tone_is_our_file,
                              NULL, tone_play,  tone_stop,      tone_pause,
                              NULL, NULL,       tone_get_time,  NULL,
                              NULL, NULL,       NULL,           NULL,
                              NULL, NULL,       tone_song_info, NULL,
                              NULL};
