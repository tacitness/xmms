...
static gint16 *get_alaw_table(void)
{
    ...
    if (!table) {
        ...
        for (i = 0; i < 128; i++) {
            ...
            table[i] = (((v & 0x0f) << 4) + (seg ? 0x108 : 8)) << (seg ? seg - 1 : 0);
        }
    }
    return table;
}
...
static gint16 *get_mulaw_table(void)
{
    ...
    if (!table) {
        ...
        table = g_malloc(128 * sizeof(gint16));
        for (i = 0; i < 128; i++) {
            table[127 - i] = ((((i & 0x0f) << 3) + 0x84) << ((i >> 4) & 7)) - 0x84;
        }
    }
    return table;
}
...
static void *play_loop(void *arg)
{
    ...
    data = g_malloc(512 * (MAX(wav_file->bits_per_sample, wav_file->bits_per_sample_out) / 8) *
                    wav_file->channels);
    while (wav_file->going) {
        if (!wav_file->eof) {
            input = MIN(blk_size, wav_file->length - wav_file->position);

            if (input > 0 && (actual_read = fread(data, 1, input, wav_file->file)) > 0) {
                output = convert(data, input);

                wav_ip.add_vis_pcm(wav_ip.output->written_time(), wav_file->afmt,
                                   wav_file->channels, output, data);
                while (wav_ip.output->buffer_free() < output && wav_file->going &&
                       wav_file->seek_to == -1)
                    xmms_usleep(10000);
...
static struct wave_file *construct_wave_file(char *filename)
{
    ...
    if (strncmp(magic, "RIFF", 4) || !read_le_long(wav->file, &len))
        goto exit_on_error;
    ...
    /* wait for fmt chunk */
    for (;;) {
        if (fread(magic, 1, 4, wav->file) != 4 || !read_le_long(wav->file, &len))
            goto exit_on_error;
    ...
    if (wav->format_tag == WAVE_FORMAT_EXTENSIBLE) {
        if (len < 24 || fseek(wav->file, 8, SEEK_CUR) ||
            !read_le_short(wav->file, &wav->format_tag))
            goto exit_on_error;
        len -= 10;
    }
    ...
    if (wav->format_tag == WAVE_FORMAT_PCM) {
        ...
        if (wav->bits_per_sample == 32 || wav->bits_per_sample == 64)
            wav->afmt = FMT_S16_LE;
        else
            goto exit_on_error;
...
static void play_file(char *filename)
{
    ...
    if (wav_ip.output->open_audio(wav_file->afmt, wav_file->samples_per_sec, wav_file->channels) ==
        0) {
        audio_error = TRUE;
        fclose(wav_file->file);
        g_free(wav_file);
    ...
    rate = wav_file->samples_per_sec * wav_file->channels * (wav_file->bits_per_sample / 8);
    wav_ip.set_info(name, 1000 * (wav_file->length / rate), 8 * rate, wav_file->samples_per_sec,
                    wav_file->channels);
    g_free(name);
    wav_file->seek_to = -1;
...
static void get_song_info(char *filename, char **title, int *length)
{
    ...
    *length =
        1000 * (wav->length / (wav->samples_per_sec * wav->channels * (wav->bits_per_sample / 8)));
...
static InputPlugin wav_ip = {
    NULL,      NULL, NULL,      plugin_init, NULL, NULL,          is_our_file, NULL,
    play_file, stop, wav_pause, seek,        NULL, get_time,      NULL,        NULL,
    NULL,      NULL, NULL,      NULL,        NULL, get_song_info,
};
