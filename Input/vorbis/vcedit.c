...
{
    vcedit_state *state = g_malloc(sizeof(vcedit_state));
...
{
    return vcedit_open_callbacks(state, (void *)in, (vcedit_read_func)fread,
                                 (vcedit_write_func)fwrite);
}

int vcedit_open_callbacks(vcedit_state *state, void *in, vcedit_read_func read_func,
                          vcedit_write_func write_func)
{
    ...
    state->os = g_malloc(sizeof(ogg_stream_state));
    ogg_stream_init(state->os, state->serial);

    vorbis_info_init(&state->vi);

    state->vc = g_malloc(sizeof(vorbis_comment));
