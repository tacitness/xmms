...
static long vorbis_process_replaygain(float **pcm, int samples, int ch, char *pcmout,
                                      float rg_scale);
...
ov_callbacks vorbis_callbacks = {ovcb_read, ovcb_seek, ovcb_close, ovcb_tell};

InputPlugin vorbis_ip = {NULL,
                         NULL,
                         NULL,              /* description */
                         vorbis_init,       /* init */
...
static int vorbis_process_data(int last_section, gboolean use_rg, float rg_scale)
{
    ...
    pthread_mutex_lock(&vf_mutex);
    if (use_rg) {
        bytes = ov_read_float(&vf, &pcm, sizeof(pcmout) / 2 / channels, &current_section);
        if (bytes > 0)
            bytes = vorbis_process_replaygain(pcm, bytes, channels, pcmout, rg_scale);
    } else
        bytes = ov_read(&vf, pcmout, sizeof(pcmout), bep, 2, 1, &current_section);
    ...
    vorbis_ip.add_vis_pcm(vorbis_ip.output->written_time(), FMT_S16_NE, channels, bytes, pcmout);
...
static void *vorbis_play_loop(void *arg)
{
    ...
    while (vorbis_playing) {
        ...
        if (current_section != last_section) {
            ...
            vorbis_ip.set_info(title, time, ov_bitrate(&vf, current_section), samplerate, channels);
            pthread_mutex_unlock(&vf_mutex);
...
static gboolean vorbis_update_replaygain(float *scale)
{
    ...
    *scale = 1.0;
    ...
    if (vorbis_cfg.use_anticlip) {
        if (vorbis_cfg.replaygain_mode == REPLAYGAIN_MODE_ALBUM)
            rg_peak_str = vorbis_comment_query(comment, "replaygain_album_peak", 0);

        if (!rg_peak_str)
            rg_peak_str = vorbis_comment_query(comment, "replaygain_track_peak", 0);
        if (!rg_peak_str)
            /* Obsolete name */
...
static char *vorbis_generate_title(OggVorbis_File *vorbisfile, char *fn)
{
    ...
    if ((comment = ov_comment(vorbisfile, -1)) != NULL) {
        input->track_name = xmms_charset_from_utf8(vorbis_comment_query(comment, "title", 0));
        input->performer = xmms_charset_from_utf8(vorbis_comment_query(comment, "artist", 0));
        input->album_name = xmms_charset_from_utf8(vorbis_comment_query(comment, "album", 0));
        if ((tmp = vorbis_comment_query(comment, "tracknumber", 0)) != NULL)
            input->track_number = atoi(tmp);
        input->date = xmms_charset_from_utf8(vorbis_comment_query(comment, "date", 0));
        if ((tmp = vorbis_comment_query(comment, "year", 0)) != NULL)
            input->year = atoi(tmp);
        input->genre = xmms_charset_from_utf8(vorbis_comment_query(comment, "genre", 0));
        /* "comment" = user comment, not "" */
        input->comment = xmms_charset_from_utf8(vorbis_comment_query(comment, "comment", 0));
    }

    displaytitle = xmms_get_titlestring(vorbis_cfg.tag_override ? vorbis_cfg.tag_format
