...
static void timeout_destroy(struct timeout *entry)
{
    g_free(entry->device);
    g_free(entry);
...
static gboolean is_mounted(char *device_name)
{
...
static void *dae_play_loop(void *arg)
{
    ...
    while (cdda_playing.playing) {
        ...
        if (dae_data.seek != -1) {
            ...
            pos = LBA(cdda_playing.cd_toc.track[cdda_playing.track]) + dae_data.seek * 75;
            dae_data.seek = -1;
            dae_data.eof = FALSE;
        }
        ...
        if (frames <= 0) {
            ...
            if (err == EOPNOTSUPP)
                 ...
                 * and such.
                 */
                g_message("read_audio_data() failed:  %s (%d)", strerror(err), err);
        ...
        while (cdda_playing.playing && left > 0 && dae_data.seek == -1) {
            ...
            cdda_ip.add_vis_pcm(cdda_ip.output->written_time(), FMT_S16_LE, 2, cur, data);
            while (cdda_ip.output->buffer_free() < cur && cdda_playing.playing &&
                   dae_data.seek == -1)
                xmms_usleep(30000);
...
static void dae_play(void)
{
    ...
    pthread_create(&dae_data.thread, NULL, dae_play_loop, NULL);
}
...
static void play_file(char *filename)
{
    ...
    if (!cdda_get_toc(&cdda_playing.cd_toc, drive->device) ||
        ...
        track < cdda_playing.cd_toc.first_track || track > cdda_playing.cd_toc.last_track)
        return;
    ...
    cdda_ip.set_info(cdda_get_title(&cdda_playing.cd_toc, track), (track_len * 1000) / 75,
                     44100 * 2 * 2 * 8, 44100, 2);
...
static char *cdda_get_title(cdda_disc_toc_t *toc, int track)
{
    ...
    cdda_cdinfo_get(&cdinfo, track, &input->performer, &input->album_name, &input->track_name);
    pthread_mutex_unlock(&title_mutex);

    input->track_number = track;
    input->file_name = input->file_path = g_strdup_printf(_("CD Audio Track %02u"), track);
    input->file_ext = "cda";
    title = xmms_get_titlestring(cdda_cfg.title_override ? cdda_cfg.name_format
...
static void drive_set_volume(int l, int r)
{
    struct cdrom_volctrl vol;
...
static gboolean cdda_get_toc_lowlevel(int fd, cdda_disc_toc_t *info)
{
...
#    ifdef XMMS_CDROM_BSD_DARWIN
    return ((subchnl.data->what.position.absaddr[1] * 60 subchnl.data->what.position.absaddr[2]) *
                75 +
...
/* Digital extraction before ATAng */
int read_audio_data(int fd, int pos, int num, void *buf)
{
...
#    elif defined(XMMS_CDROM_BSD_DARWIN)
...
static void seek(int time)
{
    ...
    start.minute = (cdda_playing.cd_toc.track[track].minute * 60 +
                    cdda_playing.cd_toc.track[track].second + time) /
                   60;
...
static int get_time_dae(void)
{
    if (dae_data.audio_error)
        return -2;
    if (!cdda_playing.playing || (dae_data.eof && !cdda_ip.output->buffer_playing()))
        return -1;
...
static void oss_set_volume(int l, int r, int mixer_line)
{
    int fd, v;
