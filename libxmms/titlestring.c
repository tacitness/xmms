 ...
 * Copyright (C) 2004,  Matti H�m�l�inen <ccr@tnsp.org>
 *
...
{
    int i;
    /* Length of the string that is actually printed */
    int plen;
...
static int parse_variable(char **fmt, GString *string, TitleInput *input)
{
    ...
    case 'n':
        exp = xmms_vputnum(string, input->track_number, &padding);
        break;
...
GtkWidget *xmms_titlestring_descriptions(char *tags, int columns)
{
    ...
    for (c = 0; c < columns; c++) {
        for (r = 0; r < (num + columns - 1 - c) / columns; r++) {
            ...
            gtk_table_attach(GTK_TABLE(table), label, 2 * c, 2 * c + 1, r, r + 1, GTK_FILL,
                             GTK_FILL, 0, 0);
