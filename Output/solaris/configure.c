...
static void configure_win_apply_cb(GtkWidget *w, gpointer data)
{
    ...
    sun_cfg.channel_flags =
        ((gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(port_chk_lineout)) ? AUDIO_LINE_OUT : 0) |
         (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(port_chk_speaker)) ? AUDIO_SPEAKER : 0) |
         (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(port_chk_headphones)) ? AUDIO_HEADPHONE
                                                                               : 0));
    sun_cfg.always_audiodev =
        gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(audio_device_always_audiodev));
    configure_write();
...
static gint scan_devices(gchar *type, GtkWidget *option_menu, GtkSignalFunc sigfunc,
                         gchar *default_name)
{
    GtkWidget *menu, *item;
    gchar *name;
    ...
    if (name) {
        ...
        gtk_signal_connect(GTK_OBJECT(item), "activate", sigfunc, default_device);
    ...
    for (index = 0; index < 10; index++) {
        ...
        if (name) {
            ...
            item = gtk_menu_item_new_with_label(tem);
            gtk_signal_connect(GTK_OBJECT(item), "activate", sigfunc, devname);
            gtk_widget_show(item);
            gtk_menu_append(GTK_MENU(menu), item);
...
void abuffer_configure(void)
{
    ...
    gtk_box_pack_start(GTK_BOX(audio_vbox), audio_device_frame, FALSE, FALSE, 0);

    audio_device_box = gtk_vbox_new(FALSE, 0);
    gtk_container_set_border_width(GTK_CONTAINER(audio_device_box), 5);
    gtk_container_add(GTK_CONTAINER(audio_device_frame), audio_device_box);

    audio_device_option = gtk_option_menu_new();
    gtk_box_pack_start(GTK_BOX(audio_device_box), audio_device_option, TRUE, TRUE, 0);
    default_index = scan_devices("Audio devices:", audio_device_option, configure_win_audio_dev_cb,
                                 sun_cfg.audio_device);
    audio_device = sun_cfg.audio_device;
    if (default_index >= 0)
        gtk_option_menu_set_history(GTK_OPTION_MENU(audio_device_option), default_index);

    audio_device_always_audiodev =
        gtk_check_button_new_with_label(_("Always use AUDIODEV environment variable"));
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(audio_device_always_audiodev),
                                 sun_cfg.always_audiodev == 1);
    gtk_box_pack_start(GTK_BOX(audio_device_box), audio_device_always_audiodev, TRUE, TRUE, 0);

    port_frame = gtk_frame_new(_("Output ports:"));

    gtk_box_pack_start(GTK_BOX(audio_vbox), port_frame, FALSE, FALSE, 0);
    ...
    gtk_notebook_append_page(GTK_NOTEBOOK(notebook), audio_vbox, gtk_label_new(_("Devices")));
