...
int init_ctlfd(void)
{
    ...
    if (strcmp(device.name, "SUNW,CS4231") && strcmp(device.name, "SUNW,sb16") &&
...
/***************************************************************************/
/* This copies data into the buffer, dependent on the endian-ness and sign */
/***************************************************************************/
void dsp_memcpy(void *dst, void *src, int len)
{
    ...
    {
        /*
         * Endian transformation required
...
void abuffer_write(void *ptr, gint length)
{
    ...
    if (effects_enabled() && ep && ep->mod_samples)
        length = ep->mod_samples(&ptr, length, input_format, stream_attrs.sample_rate,
                                 stream_attrs.channels);

    if (new_format != input_format || new_frequency != stream_attrs.sample_rate ||
        new_channels != stream_attrs.channels) {
        /* FIXME: This is in effect a flush.  Need to recalculate buffer_offset and time_offset */
