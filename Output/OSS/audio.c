...
static int oss_calc_bitrate(int oss_fmt, int rate, int channels)
{
    ...
    if (oss_fmt == AFMT_U16_BE || oss_fmt == AFMT_U16_LE || oss_fmt == AFMT_S16_BE ||
        oss_fmt == AFMT_S16_LE)
        bitrate *= 2;
...
static void oss_setup_format(AFormat fmt, int rate, int nch)
{
    ...
    output.bps = oss_calc_bitrate(output.format.oss, output.frequency, output.channels);
}
...
static void oss_write_audio(gpointer data, int length)
{
    ...
    if (new_format != effect.format.xmms || new_frequency != effect.frequency ||
    ...
    if (effect.frequency == output.frequency)
        ...
        output_bytes += oss_downsample(data, length, effect.frequency, output.frequency);
}
...
#define NOT_NATIVE_ENDIAN                                                                         \
    ((IS_BIG_ENDIAN && (output.format.oss == AFMT_S16_LE || output.format.oss == AFMT_U16_LE)) || \
     (!IS_BIG_ENDIAN && (output.format.oss == AFMT_S16_BE || output.format.oss == AFMT_U16_BE)))
...
#define RESAMPLE_STEREO(sample_type)                                                       \
    do {                                                                                   \
        ...
        for (x = 0, i = 0; i < out_samples; i++) {                                         \
            ...
            x1 = (x >> 12) << 12;                                                          \
            frac = x - x1;                                                                 \
            *outptr++ = (sample_type)((inptr[(x1 >> 12) << 1] * ((1 << 12) - frac) +       \
                                       inptr[((x1 >> 12) + 1) << 1] * frac) >>             \
                                      12);                                                 \
            *outptr++ = (sample_type)((inptr[((x1 >> 12) << 1) + 1] * ((1 << 12) - frac) + \
...
#define RESAMPLE_MONO(sample_type)                                            \
    do {                                                                      \
        ...
        for (x = 0, i = 0; i < out_samples; i++) {                            \
            ...
            x1 = (x >> 12) << 12;                                             \
            frac = x - x1;                                                    \
            *outptr++ = (sample_type)((inptr[x1 >> 12] * ((1 << 12) - frac) + \
                                       inptr[(x1 >> 12) + 1] * frac) >>       \
                                      12);                                    \
            x += delta;                                                       \
        }                                                                     \
...
void oss_write(gpointer ptr, int length)
{
    int cnt, off = 0;
...
void oss_set_audio_params(void)
{
    ...
    oss_stereo_convert_func = oss_get_stereo_convert_func(output.channels, effect.channels);
    ...
    oss_convert_func = oss_get_convert_func(output.format.oss, oss_get_format(effect.format.xmms));
...
gint oss_open(AFormat fmt, gint rate, gint nch)
{
    ...
    fd = open(device_name, O_WRONLY);
    if (fd == -1) {
        g_warning("oss_open(): Failed to open audio device (%s): %s", device_name, strerror(errno));
        g_free(device_name);
        return 0;
    }
