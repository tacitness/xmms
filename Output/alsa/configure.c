...
#define GET_SPIN_INT(spin) gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(spin))
...
void alsa_save_config(void)
{
    ...
    xmms_cfg_write_boolean(cfgfile, "ALSA", "soft_volume", alsa_cfg.soft_volume);
...
static int get_mixer_devices(GtkComboBoxText *combo, int card)
{
    ...
    while (current) {
        ...
        if (snd_mixer_selem_is_active(current) && snd_mixer_selem_has_playback_volume(current))
...
static void get_devices_for_card(GtkComboBoxText *combo, int card)
{
    ...
    for (;;) {
        ...
        if ((err = snd_ctl_pcm_next_device(ctl, &pcm_device)) < 0) {
            g_warning("snd_ctl_pcm_next_device() failed: %s", snd_strerror(-err));
            pcm_device = -1;
        }
        if (pcm_device < 0)
            break;
...
void alsa_configure(void)
{
    ...
    softvolume_toggle_button = gtk_check_button_new_with_label(_("Use software volume control"));

    gtk_box_pack_start(GTK_BOX(mixer_box), softvolume_toggle_button, FALSE, FALSE, 0);
    ...
    gtk_table_attach(GTK_TABLE(buffer_table), buffer_time_label, 0, 1, 0, 1, GTK_FILL, 0, 0, 0);
    ...
    gtk_table_attach(GTK_TABLE(buffer_table), period_time_label, 0, 1, 1, 2, GTK_FILL, 0, 0, 0);
    ...
    gtk_table_attach(GTK_TABLE(buffer_table), thread_buffer_time_label, 0, 1, 0, 1, GTK_FILL, 0, 0,
                     0);
    thread_buffer_time_adj =
