 ...
 *	Implemented the multi-threaded mode with an audio-thread.
...
static const struct {
    ...
    {FMT_S16_LE, SND_PCM_FORMAT_S16_LE}, {FMT_S16_BE, SND_PCM_FORMAT_S16_BE},
...
static snd_pcm_sframes_t alsa_get_avail(void)
{
    ...
    while ((ret = snd_pcm_avail_update(alsa_pcm)) < 0) {
        ...
        if (ret < 0) {
            g_warning("alsa_get_avail(): snd_pcm_avail_update() failed: %s", snd_strerror(-ret));
            return 0;
        }
    }
    return ret;
...
int alsa_get_mixer(snd_mixer_t **mixer, int card)
{
    ...
    if ((err = snd_mixer_open(mixer, 0)) < 0) {
        g_warning("alsa_get_mixer(): Failed to open empty mixer: %s", snd_strerror(-err));
        mixer = NULL;
        return -1;
    }
    ...
    if ((err = snd_mixer_attach(*mixer, dev)) < 0) {
        g_warning("alsa_get_mixer(): Attaching to mixer %s failed: %s", dev, snd_strerror(-err));
        g_free(dev);
        return -1;
    }
    g_free(dev);
    ...
    if ((err = snd_mixer_selem_register(*mixer, NULL, NULL)) < 0) {
        g_warning("alsa_get_mixer(): Failed to register mixer: %s", snd_strerror(-err));
        return -1;
    }
    if ((err = snd_mixer_load(*mixer)) < 0) {
        g_warning("alsa_get_mixer(): Failed to load mixer: %s", snd_strerror(-err));
        return -1;
    }
...
static int alsa_setup_mixer(void)
{
    ...
    if (!pcm_element) {
        g_warning("alsa_setup_mixer(): Failed to find mixer element: %s", alsa_cfg.mixer_device);
        return -1;
    }
...
#define STEREO_ADJUST(type, type2, endian)                                        \
    do {                                                                          \
        ...
        for (i = 0; i < length; i += 4) {                                         \
            *ptr = type2##_TO_##endian(type2##_FROM_##endian(*ptr) * lvol / 256); \
            ptr++;                                                                \
            *ptr = type2##_TO_##endian(type2##_FROM_##endian(*ptr) * rvol / 256); \
            ptr++;                                                                \
        }                                                                         \
    } while (0)
...
#define MONO_ADJUST(type, type2, endian)                                         \
    do {                                                                         \
        ...
        for (i = 0; i < length; i += 2) {                                        \
            *ptr = type2##_TO_##endian(type2##_FROM_##endian(*ptr) * vol / 256); \
            ptr++;                                                               \
        }                                                                        \
    } while (0)
...
static void volume_adjust(void *data, int length, AFormat fmt, int channels)
{
    ...
    if ((alsa_cfg.vol.left == 100 && alsa_cfg.vol.right == 100) ||
        (channels == 1 && (alsa_cfg.vol.left == 100 || alsa_cfg.vol.right == 100)))
        return;
...
static void alsa_do_write(gpointer data, int length)
{
    ...
    if (ep)
        length =
            ep->mod_samples(&data, length, inputf->xmms_format, inputf->rate, inputf->channels);
    ...
    if (alsa_frequency_convert_func != NULL)
        length = alsa_frequency_convert_func(convertb, &data, length, effectf->rate, outputf->rate);
    ...
    while (going && alsa_pcm) {
        ...
        if (!paused && !prebuffer && get_thread_buffer_filled() > hw_period_size_in) {
            ...
            if (poll(pfds, npfds, 10) > 0) {
                 ...
                 * dmix returns a positive value even
...
static int alsa_setup(struct snd_format *f)
{
    ...
    if ((err = snd_pcm_open(&alsa_pcm, alsa_cfg.pcm_device, SND_PCM_STREAM_PLAYBACK,
                            SND_PCM_NONBLOCK)) < 0) {
        g_warning("alsa_setup(): Failed to open pcm device (%s): %s", alsa_cfg.pcm_device,
                  snd_strerror(-err));
        alsa_pcm = NULL;
        g_free(outputf);
        outputf = NULL;
    ...
    if ((err = snd_pcm_hw_params_set_access(alsa_pcm, hwparams, SND_PCM_ACCESS_RW_INTERLEAVED)) <
        0) {
        g_warning("alsa_setup(): Cannot set direct write mode: %s", snd_strerror(-err));
        return -1;
    }
    ...
    if ((err = snd_pcm_hw_params_set_format(alsa_pcm, hwparams, outputf->format)) < 0) {
         ...
         */
        snd_pcm_format_t formats[] = {SND_PCM_FORMAT_S16_LE, SND_PCM_FORMAT_S16_BE,
                                      SND_PCM_FORMAT_U8};
        int i;
        ...
        if (outputf->format != f->format) {
            ...
            debug("Converting format from %d to %d", f->xmms_format, outputf->xmms_format);
    ...
    if (outputf->channels != f->channels) {
        debug("Converting channels from %d to %d", f->channels, outputf->channels);
        alsa_stereo_convert_func =
            xmms_convert_get_channel_func(outputf->xmms_format, outputf->channels, f->channels);
        if (alsa_stereo_convert_func == NULL)
            return -1;
    }
    ...
    if (outputf->rate != f->rate) {
        debug("Converting samplerate from %d to %d", f->rate, outputf->rate);
        alsa_frequency_convert_func =
            xmms_convert_get_frequency_func(outputf->xmms_format, outputf->channels);
        if (alsa_frequency_convert_func == NULL)
            return -1;
    }
    ...
    if ((err = snd_pcm_hw_params_set_buffer_time_near(alsa_pcm, hwparams, &alsa_buffer_time, 0)) <
        0) {
        g_warning("alsa_setup(): Set buffer time failed: %s.", snd_strerror(-err));
        return -1;
    }

    alsa_period_time = alsa_cfg.period_time * 1000;
    if ((err = snd_pcm_hw_params_set_period_time_near(alsa_pcm, hwparams, &alsa_period_time, 0)) <
        0) {
        g_warning("alsa_setup(): Set period time failed: %s.", snd_strerror(-err));
        return -1;
    }
    ...
    if ((err = snd_pcm_sw_params_set_start_threshold(alsa_pcm, swparams,
                                                     alsa_buffer_size - alsa_period_size) < 0))
    ...
    if (inputf->bps != outputf->bps) {
        ...
        hw_buffer_size_in =
            ((guint64)hw_buffer_size * inputf->bps + outputf->bps / 2) / outputf->bps;
        hw_period_size_in =
            ((guint64)hw_period_size * inputf->bps + outputf->bps / 2) / outputf->bps;
    ...
    debug("bits per sample: %i; frame size: %i; Bps: %i",
          snd_pcm_format_physical_width(outputf->format), snd_pcm_frames_to_bytes(alsa_pcm, 1),
          outputf->bps);
