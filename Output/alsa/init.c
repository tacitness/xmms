...
void alsa_init(void)
{
    ...
    if (!xmms_cfg_read_string(cfgfile, "ALSA", "pcm_device", &alsa_cfg.pcm_device))
    ...
    if (!xmms_cfg_read_string(cfgfile, "ALSA", "mixer_device", &alsa_cfg.mixer_device))
        alsa_cfg.mixer_device = g_strdup("Master"); /* fix(#12): PCM is absent on most modern ALSA setups */
    ...
    alsa_cfg.thread_buffer_time =
        CLAMP(alsa_cfg.thread_buffer_time, THREAD_BUFFER_TIME_MIN, THREAD_BUFFER_TIME_MAX);

    xmms_cfg_read_boolean(cfgfile, "ALSA", "soft_volume", &alsa_cfg.soft_volume);
