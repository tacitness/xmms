...
static void sun_setformat(AFormat fmt, int rate, int nch)
{
    ...
    output.bps = sun_bps(output.format.sun, output.frequency, output.channels);
...
void sun_setparams(void)
{
    ...
    if (ioctl(audio.fd, AUDIO_SETINFO, &info) != 0) {
        g_error("%s: cannot play (%s)", audio.devaudio, strerror(errno));
        return;
    }
     ...
     */
    enc.index = 0;
    while (ioctl(audio.fd, AUDIO_GETENC, &enc) == 0 && enc.encoding != output.format.sun)
        enc.index++;
    ...
    if (ioctl(audio.fd, AUDIO_SETINFO, &info) != 0) {
        g_error("%s: unsupported encoding: %s (%s)", audio.devaudio, output.name, strerror(errno));
        return;
    }
    ...
    if (ioctl(audio.fd, AUDIO_SETINFO, &info) < 0) {
        g_error("%s: cannot handle %i Hz (%s)", audio.devaudio, output.frequency, strerror(errno));
        return;
    }
    ...
    sun_convert = sun_get_convert_func(output.format.sun, sun_format(effect.format.xmms));
#if 0
	if (sun_convert != NULL)
	{
...
static inline void sun_write_audio(gpointer data, int length)
{
    ...
    if (effect.frequency == output.frequency) {
        ...
        output_bytes += sun_downsample(data, length, effect.frequency, output.frequency);
    }
}
        ...
        if (sun_used() > 0 && !audio.paused && !prebuffer) {
            ...
            if (select(audio.fd + 1, NULL, &set, NULL, &tv) > 0) {
                ...
                while (length > 0) {
                    ...
                    sun_write_audio(buffer + rd_index, cnt);
