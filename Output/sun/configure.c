...
static void configure_win_ok_cb(GtkWidget *w, gpointer data)
{
    ...
    audio.req_buffer_size = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(buffer_size_spin));
    audio.req_prebuffer_size = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(buffer_pre_spin));

    if (sun_mixer_open() == 0) {
        audio.mixer_keepopen = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(keepopen_cbutton));
        sun_mixer_close();
    }

    cfgfile = xmms_cfg_open_default_file();

    xmms_cfg_write_string(cfgfile, "sun", "audio_devaudio", audio.devaudio);
    xmms_cfg_write_string(cfgfile, "sun", "audio_devaudioctl", audio.devaudioctl);
    xmms_cfg_write_string(cfgfile, "sun", "audio_devmixer", audio.devmixer);

    xmms_cfg_write_string(cfgfile, "sun", "mixer_voldev", audio.mixer_voldev);
    xmms_cfg_write_boolean(cfgfile, "sun", "mixer_keepopen", audio.mixer_keepopen);

    xmms_cfg_write_int(cfgfile, "sun", "buffer_size", audio.req_buffer_size);
    xmms_cfg_write_int(cfgfile, "sun", "prebuffer_size", audio.req_prebuffer_size);
...
static void configure_mixer_volumedev_scan(gchar *type, GtkWidget *option_menu)
{
    ...
    for (info.index = 0; ioctl(audio.mixerfd, AUDIO_MIXER_DEVINFO, &info) == 0; info.index++) {
        GtkWidget *item;

        if (info.type == AUDIO_MIXER_VALUE) {
            item = gtk_menu_item_new_with_label(info.label.name);
            gtk_signal_connect(GTK_OBJECT(item), "activate", configure_win_mixer_volume_dev_cb,
                               (gpointer)info.index);
...
static void configure_actldevice_box(GtkWidget *dev_vbox)
{
    ...
    gtk_box_pack_start(GTK_BOX(dev_vbox), actldevice_frame, FALSE, FALSE, 0);
...
static void configure_buffering_frame(GtkWidget *vbox, GtkWidget *notebook)
{
    ...
    buffer_size_adj = gtk_adjustment_new(audio.req_buffer_size, 200, 131072, 100, 100, 100);
    ...
    buffer_pre_adj = gtk_adjustment_new(audio.req_prebuffer_size, 0, 90, 1, 1, 1);
...
static void configure_mixer_toggle_button(GtkWidget *vbox, gchar *devname, gchar *label)
{
    ...
    if (!sun_mixer_get_dev(audio.mixerfd, &devid, devname)) {
        ...
        if (!ioctl(audio.mixerfd, AUDIO_MIXER_READ, &mixer)) {
            toggle_cbutton = gtk_check_button_new_with_label(_(label));
            gtk_box_pack_start_defaults(GTK_BOX(vbox), toggle_cbutton);

            if (mixer.un.ord) {
                gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(toggle_cbutton), TRUE);
                mixer_toggle[mixer.dev]++;
            } else {
                mixer_toggle[mixer.dev] = 0;
            }

            gtk_signal_connect(GTK_OBJECT(toggle_cbutton), "toggled",
                               GTK_SIGNAL_FUNC(mixer_cbutton_toggled_cb), (gpointer)mixer.dev);
        }
    }
}
...
static void configure_mixer_box(GtkWidget *vbox, GtkWidget *notebook)
{
    ...
    keepopen_cbutton = gtk_check_button_new_with_label(_("XMMS uses mixer exclusively."));
    if (audio.mixer_keepopen)
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(keepopen_cbutton), TRUE);
...
static void configure_mixer_frame(GtkWidget *vbox, GtkWidget *notebook)
{
    ...
    gtk_notebook_append_page(GTK_NOTEBOOK(notebook), mixervol_vbox, gtk_label_new(_("Mixer")));
}
    ...
    while (stats_frame.active && stats_frame.fd) {
        ...
        if (!ioctl(stats_frame.fd, AUDIO_GETINFO, &info)) {
            ...
            if (info.mode == AUMODE_PLAY) {
                sprintf(s, "%s at %i Hz (%i-bit %s)", s, info.play.sample_rate, info.play.precision,
                        audio.output->name);
                sprintf(sl, "%i samples, %i error(s). %s", info.play.samples, info.play.error,
                        info.play.active ? "I/O in progress." : "");
            }
            gtk_label_set_text(GTK_LABEL(stats_frame.mode_label), s);

            sprintf(s, "H/W block: %i bytes. Buffer: %i bytes", info.blocksize,
                    info.play.buffer_size);
            gtk_label_set_text(GTK_LABEL(stats_frame.blocksize_label), s);
        }
...
static void configure_status_frame(GtkWidget *vbox, GtkWidget *notebook)
{
    ...
    gtk_container_add(GTK_CONTAINER(status_vbox), stats_frame.blocksize_label);
    stats_frame.ooffs_label = gtk_label_new(NULL);
    gtk_container_add(GTK_CONTAINER(status_vbox), stats_frame.ooffs_label);

    gtk_notebook_append_page(GTK_NOTEBOOK(notebook), status_vbox, gtk_label_new(_("Status")));
    ...
    if ((stats_frame.fd = open(audio.devaudioctl, O_RDWR)) >= 0) {
        ...
        if (ioctl(stats_frame.fd, AUDIO_GETDEV, &device) >= 0) {
            char *s = g_strdup_printf("%s - %s(4) %s", device.name, device.config, device.version);
        ...
        if (ioctl(stats_frame.fd, AUDIO_GETPROPS, &props) >= 0) {
            ...
            if ((props & AUDIO_PROP_FULLDUPLEX) == AUDIO_PROP_FULLDUPLEX)
                sprintf(s, "FULLDUPLEX ");
            if ((props & AUDIO_PROP_MMAP) == AUDIO_PROP_MMAP)
                sprintf(s, "%s MMAP ", s);
            if ((props & AUDIO_PROP_INDEPENDENT) == AUDIO_PROP_INDEPENDENT)
...
void sun_configure(void)
{
    ...
    gtk_signal_connect(GTK_OBJECT(configure_win), "destroy", GTK_SIGNAL_FUNC(configure_win_destroy),
                       NULL);

    gtk_window_set_title(GTK_WINDOW(configure_win), _("Sun driver configuration"));
    ...
    gtk_signal_connect_object(GTK_OBJECT(cancel), "clicked",
                              GTK_SIGNAL_FUNC(configure_win_cancel_cb), GTK_OBJECT(configure_win));
